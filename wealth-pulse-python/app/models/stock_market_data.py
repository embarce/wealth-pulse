...
class StockMarketData(Base):
    ...
    day_low: Mapped[float | None] = mapped_column(Float(20), comment="日内最低(L)")
    day_high: Mapped[float | None] = mapped_column(Float(20), comment="日内最高(N)")
    price_range_percent: Mapped[float | None] = mapped_column(Float(10), comment="价格区间百分比")
    ...
    bid_price: Mapped[float | None] = mapped_column(Float(20), comment="买一价")
    ask_price: Mapped[float | None] = mapped_column(Float(20), comment="卖一价")
    bid_size: Mapped[int | None] = mapped_column(BigInteger, comment="买一量")
    ask_size: Mapped[int | None] = mapped_column(BigInteger, comment="卖一量")
